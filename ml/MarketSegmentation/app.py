...
# if submitted:
#     clust = loaded_model.predict(data)[0]
#     st.write(f'Data Belongs to Cluster {clust}')  # Use st.write() instead of print()

#     cluster_df1 = df[df['Cluster'] == clust]
#     for c in cluster_df1.drop(['Cluster'], axis=1):
#         fig, ax = plt.subplots()  # No need to set figsize here if using FacetGrid
#         grid = sns.FacetGrid(cluster_df1, col='Cluster')
#         grid.map(plt.hist, c)
...
if submitted:
    ...
    cluster_df1 = df[df['Cluster'] == clust]
    features_to_plot = cluster_df1.drop(['Cluster'], axis=1).columns
    number_of_subplots = len(features_to_plot)

    # Set the figure size dynamically based on the number of subplots
    fig, axes = plt.subplots(number_of_subplots, 1, figsize=(20, 3 * number_of_subplots))

    for index, feature in enumerate(features_to_plot):
        # Plot on the respective subplot
        sns.histplot(cluster_df1[feature], ax=axes[index], kde=True)
        axes[index].set_title(f'Histogram of {feature}')
